...
class Player:
    ...
    def __repr__(self):
        return f"Player(name={self.name}, rating={self.rating:.1f}, matches={self.matches})"
...
class EloCalculator:
    ...
    def calculate_new_rating(self, player, opponent, result):
        expected = self.calculate_expected_score(player.rating, opponent.rating)
        return player.rating + self.k_factor * (result - expected)

    def update_ratings(self, match):
        new_rating1 = self.calculate_new_rating(match.player1, match.player2, match.result)
        new_rating2 = self.calculate_new_rating(match.player2, match.player1, 1 - match.result)
