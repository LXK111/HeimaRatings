...
- 阶段 30 的目标是把比赛项目管理从只读推进到真实创建和编辑。
- 比赛项目是签表、比赛结果和项目级排名的直接挂载点。
- 写入路径统一通过 Repository，不在页面中直接依赖 Supabase client。
- 管理 API 写接口继续复用阶段 22/24 的 `admin/editor` 写权限规则。
- 页面表单使用 Server Actions 调用请求级 Repository，让 Supabase 模式继续走当前用户 JWT 和数据库 RLS。
...
- 已更新 `HeimaRatings/lib/server/repositories/types.ts`，新增比赛项目创建和编辑契约。
- 已更新 `HeimaRatings/lib/server/repositories/supabase.ts`，新增 `tournament_events` 创建和编辑持久化路径。
- 已更新 `HeimaRatings/lib/server/repositories/mock.ts`，补齐 Mock Repository 写入接口。
- 已更新 `HeimaRatings/app/api/tournaments/[id]/events/route.ts`，新增 `POST` 和 `PATCH`。
- 已新增 `HeimaRatings/lib/server/tournament-event-actions.ts`，为页面表单提供 Server Actions。
- 已更新 `HeimaRatings/app/tournaments/[id]/events/page.tsx`，新增创建表单和行内编辑表单。
- 已创建 `HeimaRatings/docs/stage-30.md`。
- 已更新 `HeimaRatings/README.md` 当前阶段和后续阶段边界。
...
- `matches` 继续表示真实比赛，`bracket_slots` 表示签表位置。
- 单败淘汰生成时，真实对阵选手写入相邻 `occupied` slots。
- 奇数人数轮空选手写入 `bye` slot，不创建虚拟 match。
- 已有 `matches` 或已有 `bracket_slots` 时拒绝重复生成，避免覆盖签表。
- 本阶段不改变签表页面读取逻辑，也不改晋级落位逻辑。

代码生成记录：

- 已更新 `HeimaRatings/lib/server/repositories/supabase.ts`，初始单败签表生成同步写入 `bracket_slots`。
- 已更新 `HeimaRatings/lib/server/mock-repository.ts` 和 `HeimaRatings/lib/server/repositories/mock.ts`，Mock 生成路径同步维护内存 slots。
- 已更新 `HeimaRatings/lib/domain/types.ts`，新增 `BracketSlotSummary`。
- 已新增 `HeimaRatings/scripts/verify_supabase_bracket_slots_generation.mjs`。
- 已更新 `HeimaRatings/package.json`，新增 `npm run bracket:slots:verify`。
- 已创建 `HeimaRatings/docs/stage-52.md`。
- 已更新 `HeimaRatings/README.md` 当前阶段、运行命令、阶段边界和后续阶段。
...
### 待执行阶段整理：部署前主线与交互业务优化项

方案细节：

- 当前工程已推进到阶段 54，签表生成、晋级落位和管理端签表视图读取都已接入 `bracket_slots`。
- 阶段 56 和阶段 57 属于签表操作与视觉体验增强，不阻塞基础数据闭环和部署验收。
- 因此将阶段 56、阶段 57 标记为“交互业务优化项（部署后推进）”，避免在部署前主线中扩大交互复杂度。

部署前主线：

- 阶段 55：签表视图浏览器验收。用真实浏览器验证管理端页面展示 `bracket_slots` 中的固定签位、轮空和晋级来源，并覆盖 viewer/editor 权限边界。
- 阶段 58：真实数据导入/初始化工具。支持从 CSV 或表格批量导入选手、武器积分、赛事参赛名单和初始数据。
...
- 阶段 56：slot 编辑能力。支持人工调整签位、种子落位、轮空位和冲突校验。
- 阶段 57：完整签表页面体验优化。增强 bracket 视觉层级、来源关系、冠军位和轮空展示。
...
- 已新增 `HeimaRatings/scripts/import_seed_data.mjs`。
- 已新增 `HeimaRatings/scripts/verify_data_import.mjs`。
- 已更新 `HeimaRatings/package.json`，新增 `npm run data:import` 和 `npm run data:import:verify`。
