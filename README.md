...
- `npm run auth:browser:verify` 会以 Supabase 模式启动临时服务，用真实 viewer/editor 测试账号验证浏览器登录态和 viewer 写权限拒绝。
...
- 后续增强：完善自动轮空落位模型。
- 后续增强：将 Supabase 浏览器验收扩展到真实表单提交路径。
